...
LightMem is a Codex CLI and Claude Code plugin that turns any repo into a structured, team-shareable, gateway-routed project memory database. This repository is the plugin's source; **do not install LightMem into this repo itself.**
...
Reference documentation (lifecycle contract, not loaded by agents): [`hooks/memory-persistence/`](./hooks/memory-persistence/).
...
- Skill files live under `skills/<name>/SKILL.md` with full namespace `/lightmem:<name>`.
