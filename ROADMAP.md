...
> **Status:** v0.3.0 current. Verify with `python3 -W error -m unittest discover tests`.
...
- 3 initial skills: `/lightmem:init` (interactive bootstrap, 3-option CLAUDE.md handling), `/lightmem:doctor` (17 integrity checks), `/lightmem:index` (regenerate topic index).
...
- Unit, integration, and plugin wiring tests via real subprocesses.
- 7 ECC-derived files individually attributed in [`LICENSE-NOTICE.md`](./LICENSE-NOTICE.md).

---

## v0.1.1 — hardening (released)
...
## v0.2.0 — manual curation (released)
...
Direct writes to `CLAUDE.md` or `AGENTS.md` bodies are forbidden for durable project facts.
...
- **`/lightmem:update`** — reads inbox + Claude Code project memory or user-selected Codex repo notes, presents candidates, user confirms per-item, writes to `topics/`, patches `index.md` incrementally.
...
- Worktree-aware resume-injection (C5 above) remains a hardening target.

---

## v0.3.0 — Codex adapter (current)
...
## v0.4.0 — retrieval + opt-in background curator
...
- **Cross-project / global memory aggregation.** LightMem is repo-local. User cross-project preferences belong in the agent's global memory or global instructions.
